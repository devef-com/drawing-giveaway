...
export function DrawingStats({ drawingId, className }: DrawingStatsProps) {
  ...
  const stats = [
    {
      label: m.drawing_stats_available(),
      value: data.available,
      color: 'text-green-600 dark:text-green-400',
      bgColor: 'bg-green-100 dark:bg-green-900/20',
    },
    {
      label: m.drawing_stats_taken(),
      value: data.taken,
      color: 'text-red-600 dark:text-red-400',
      bgColor: 'bg-red-100 dark:bg-red-900/20',
    },
    {
      label: m.drawing_stats_reserved(),
      value: data.reserved,
    ...
    <div className={cn('rounded-lg border bg-card p-6 space-y-4', className)}>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm">
          <span className="font-medium">{m.drawing_stats_progress()}</span>
      ...
      <div className="pt-2 border-t text-center">
        <div className="text-sm text-muted-foreground">
          {m.drawing_stats_totalNumbers()}: <span className="font-semibold">{data.total}</span>
        </div>
      </div>
    </div>
  )
